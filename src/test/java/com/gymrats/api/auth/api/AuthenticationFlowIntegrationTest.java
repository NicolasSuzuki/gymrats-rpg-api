...
class AuthenticationFlowIntegrationTest {
  ...
  void registersAuthenticatesAndReturnsCurrentUser() throws Exception {
    ...
    mvc.perform(get("/api/v1/users/me").header("Authorization", "Bearer " + registrationToken))
        .andExpect(status().isOk()).andExpect(jsonPath("$.email").value("nicolas@example.com")).andExpect(jsonPath("$.role").value("USER"));
  ...
  void rejectsDuplicateEmailAndAnonymousAccess() throws Exception {
    ...
    mvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON).content(body)).andExpect(status().isConflict()).andExpect(jsonPath("$.code").value("EMAIL_ALREADY_USED"));
